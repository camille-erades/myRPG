...
static void type_idle_next_b1(mob_t *b1, game_t *game, int type, screen_t *hud)
{
    if (b1->ani_index >= 10)
        b1->ani_index = 0;
    sfSprite_setTextureRect(b1->sprite,
    (sfIntRect){b1->ani_index * 145, 0, 145, 146.25});
...
void type_idle_b1(mob_t *b1, game_t *game, int type, screen_t *hud)
{
    if (type == 0){
        ...
        if (sfTime_asSeconds(b1->ani_time) > 0.1){
            b1->ani_index++;
            type_idle_next_b1(b1, game, type, hud);
        }
    }
}

static void type_idle_next_m(mob_t *m, game_t *game, int type, screen_t *hud)
{
    if (m->ani_index >= 8)
        m->ani_index = 0;
    sfSprite_setTextureRect(m->sprite,
    (sfIntRect){m->ani_index * 250, 0, 250, 151});
...
void type_idle_m(mob_t *m, game_t *game, int type, screen_t *hud)
{
    if (type == 0){
        ...
        if (sfTime_asSeconds(m->ani_time) > 0.2){
            m->ani_index++;
            type_idle_next_m(m, game, type, hud);
        }
    }
}
